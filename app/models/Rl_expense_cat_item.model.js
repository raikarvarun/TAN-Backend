...
const DataModel = function(arg) {
  this.expenseCategaryID = arg.expenseCategaryID;
  this.expenseItemID = arg.expenseItemID;
  this.date = arg.date;
...
DataModel.create = (newData, result) => {
  sql.query("INSERT INTO Rl_expense_cat_item SET ?", newData, (err, res) => {
    if (err) {
...
DataModel.getAll = (adminid, result) => {
  ...
  sql.query("SELECT * FROM Rl_expense_cat_item WHERE adminID = ?", adminid, (err, res) => {
    if (err) {
      // console.log("error: ", err);
      result(null, err);
      return;

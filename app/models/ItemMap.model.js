...
const DataModel = function(arg) {
  this.baseUnitID = arg.baseUnitID;
  this.secondUnitID = arg.secondUnitID;
  this.rate = arg.rate;
...
DataModel.create = (newData, result) => {
  sql.query("INSERT INTO ItemMap SET ?", newData, (err, res) => {
    if (err) {
    ...
    result(null, { mapID: res.insertId, ...newData });
  });
};
...
DataModel.getAll = (adminid, result) => {
  ...
  sql.query("SELECT * FROM ItemMap WHERE adminID = ?", adminid, (err, res) => {
    if (err) {
      // console.log("error: ", err);
      result(null, err);
      return;
